...
**Unconfirmed — verify before wiring the live call, not assumed:** the exact query parameter names/values for requesting season-long ("draft") projections versus a specific week, the scoring-format parameter's name and accepted values (`STD`/`HALF`/`PPR` or equivalent), and the precise response field name holding projected fantasy points. FantasyPros' interactive API docs (`https://api.fantasypros.com/public/v2/docs`) are the source of truth here; confirm against a live request during implementation before trusting the shape sketched above.
